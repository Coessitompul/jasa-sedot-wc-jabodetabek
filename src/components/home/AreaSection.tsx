        ...
        <AnimateIn animation="fade-up" delay={100}>
          <div className="flex flex-wrap justify-center gap-3 mb-10">
            {areaChip.map((chip) => {
              ...
              const base = "flex items-center gap-1.5 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-200"
              if (slug) {
                return (
                  <Link
                    key={chip}
                    href={`/area-layanan/${slug}`}
                    className={`${base} bg-navy text-white hover:bg-navy-light hover:scale-105`}
                  >
              ...
              return (
                <span
                  key={chip}
                  className={`${base} bg-white text-slate-700 border border-slate-200 hover:border-navy hover:text-navy cursor-default`}
                >
                  <MapPin className="w-3.5 h-3.5" />
                  {chip}
                </span>
