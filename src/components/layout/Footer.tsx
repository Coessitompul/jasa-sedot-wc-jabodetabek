import Link from "next/link"
import { Phone, Mail, MapPin, Clock } from "lucide-react"
import { COMPANY, getWALink } from "@/lib/constants"
import { layananList } from "@/data/layanan"
import { areaList } from "@/data/area"

export default function Footer() {
  return (
    <footer className="bg-navy text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
          {/* Kolom 1: Tentang */}
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-9 h-9 rounded-lg bg-brand-orange flex items-center justify-center">
                <span className="text-white font-bold">BJ</span>
              </div>
              <div>
                <p className="font-display font-bold text-white leading-tight">{COMPANY.name}</p>
              </div>
            </div>
            <p className="text-slate-300 text-sm leading-relaxed mb-5">
              Jasa sedot WC & tinja profesional se-Jabodetabek. Berdiri sejak {COMPANY.established}, melayani {COMPANY.totalCustomers} pelanggan dengan armada lengkap dan teknisi berpengalaman.
            </p>
            <div className="flex items-center gap-3">
              <a
                href={COMPANY.instagram}
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg bg-navy-light flex items-center justify-center hover:bg-brand-orange transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              <a
                href={COMPANY.facebook}
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg bg-navy-light flex items-center justify-center hover:bg-brand-orange transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            </div>
          </div>

          {/* Kolom 2: Layanan */}
          <div>
            <h4 className="font-display font-bold text-white mb-4">Layanan Kami</h4>
            <ul className="space-y-2">
              {layananList.map((l) => (
                <li key={l.slug}>
                  <Link
                    href={`/layanan#${l.slug}`}
                    className="text-slate-300 text-sm hover:text-brand-orange transition-colors"
                  >
                    {l.nama}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Kolom 3: Area */}
          <div>
            <h4 className="font-display font-bold text-white mb-4">Area Layanan</h4>
            <ul className="space-y-2">
              {areaList.slice(0, 5).map((a) => (
                <li key={a.slug}>
                  <Link
                    href={`/area-layanan/${a.slug}`}
                    className="text-slate-300 text-sm hover:text-brand-orange transition-colors"
                  >
                    {a.nama}
                  </Link>
                </li>
              ))}
              <li>
                <Link
                  href="/area-layanan"
                  className="text-brand-orange text-sm hover:underline"
                >
                  Lihat Semua Area →
                </Link>
              </li>
            </ul>
          </div>

          {/* Kolom 4: Kontak */}
          <div>
            <h4 className="font-display font-bold text-white mb-4">Kontak Kami</h4>
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                <MapPin className="w-4 h-4 text-brand-orange shrink-0 mt-0.5" />
                <span className="text-slate-300 text-sm">{COMPANY.address}</span>
              </li>
              <li className="flex items-center gap-3">
                <Clock className="w-4 h-4 text-brand-orange shrink-0" />
                <span className="text-slate-300 text-sm">24 Jam / 7 Hari</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="w-4 h-4 text-brand-orange shrink-0" />
                <a href={`tel:${COMPANY.phoneTel}`} className="text-slate-300 text-sm hover:text-white transition-colors">
                  {COMPANY.phone}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="w-4 h-4 text-brand-orange shrink-0" />
                <a href={`mailto:${COMPANY.email}`} className="text-slate-300 text-sm hover:text-white transition-colors break-all">
                  {COMPANY.email}
                </a>
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-navy-light mt-8 md:mt-12 pt-6 md:pt-8 flex flex-col sm:flex-row items-center justify-between gap-3 md:gap-4">
          <p className="text-slate-400 text-sm text-center sm:text-left">
            © 2026 {COMPANY.name}. All rights reserved.
          </p>
          {/* <div className="flex items-center gap-4">
            <a href={getWALink()} className="text-slate-400 text-sm hover:text-white transition-colors">
              Kebijakan Privasi
            </a>
            <a href={getWALink()} className="text-slate-400 text-sm hover:text-white transition-colors">
              Syarat & Ketentuan
            </a>
          </div> */}
        </div>
      </div>
    </footer>
  )
}
